...
// El proxy exige el tag con "#" y URL-encodeado una sola vez. Como el usuario
// lo escribe a mano ("2pp", "#2PP", " 2pp "), la normalizacion es la parte
// del cliente que mas facil se rompe.
describe('cliente del proxy de Clash of Clans', () => {
  beforeEach(() => {
  ...
  describe('normalizacion del tag', () => {
    it('agrega el # cuando falta', async () => {
      ...
      expect(urlLlamada()).toBe('/api/clan/%232PP');
    });
  ...
  describe('rutas', () => {
    it('consulta el detalle de un jugador', async () => {
      ...
      expect(urlLlamada()).toBe('/api/player/%23LJ8V90G0');
    });

    it('consulta la guerra actual de un clan', async () => {
      await fetchCurrentWar('2PP');

      expect(urlLlamada()).toBe('/api/clan/%232PP/currentwar');
    });
  ...
  describe('errores', () => {
    it('usa el mensaje que manda el proxy', async () => {
      global.fetch = vi.fn().mockResolvedValue({
        ok: false,
        status: 403,
