...
Además del Sheet, el proyecto incluye un **proxy mínimo en `server/`** que
...
`server/.env` y solo expone JSON al frontend.
...
5. Creá `server/.env` a partir del ejemplo y pegá el token:
   ```bash
...
simultáneas (in-flight). Cambialo con `CACHE_TTL_MS` en `server/.env`.
